...
**Plan ID:** FATH-BOOTSTRAP-PLAN v6 (review sequence 2 under AMENDMENT-002 — adjudications in `REMEDIATION-1_ADJUDICATION.md`, `REMEDIATION-2_ADJUDICATION.md`, `V4_ADJUDICATION.md`, `REMEDIATION-3_ADJUDICATION.md`, `REMEDIATION-4_ADJUDICATION.md`)
...
**Status:** PLAN_READY_FOR_INDEPENDENT_REVIEW (review sequence 2, plan_review_attempt **3 of 3 — FINAL; a rejection becomes BLOCKED_FOR_SALIM** — sequence 2 authorized by Escalation Policy §6 after human-approved AMENDMENT-002 resolved FATH-P3-001; seq2 attempt-2 REJECT record: `.delivery/reviews/FATH-BOOTSTRAP/plan-review-seq2-attempt-2.md`, findings FATH-V5-001..005 adjudicated and corrected in `REMEDIATION-4_ADJUDICATION.md`; earlier records preserved unchanged) — this plan is NOT self-approved and authorizes no implementation until independent Sol review returns APPROVE.
...
| gitleaks (pinned by exact version + verified sha256 checksum of the downloaded binary) as the secret-scanning tool in CI | No project doc names a scanner; doc 28 + SG-TR-006 require the outcome. Reversible (FATH-PR-003 A9 oracle). |
...
- **Runtime services (local/dev):** docker-compose with `postgres` (custom PG16+AGE+pgvector image, base pinned by digest — PROPOSED §4.4) and **Redis 7 pinned by immutable digest** (`redis:<7.x.y>@sha256:<digest>` recorded in the compose file — a bare `redis:7` tag is mutable and would let the same candidate SHA run against different image contents; FATH-V5-004). MinIO added by the Raw-Archive task; Prefect/vLLM/sandbox services added when their tasks arrive. (SPECIFIED set; incremental introduction is DERIVED from bounded-task discipline.) Running-version verification (PG major 16; Redis major 7) is a TASK-001 acceptance criterion (A2).
- **Python:** uv-managed `pyproject.toml` (`requires-python = ">=3.11,<3.12"` — PROPOSED baseline) + committed `uv.lock` whose freshness is proven by `uv lock --check` (official uv semantics; `--frozen` does not check freshness — FATH-V4-004). TASK-001 dependency baseline is minimal and closed as a **three-set contract** (REQUIRED / OPTIONAL / PROHIBITED with positive completeness — exact lists: TASK-001_PLAN §4.9, mechanically checked by A15): REQUIRED pydantic v2, pydantic-settings, SQLAlchemy 2, alembic, asyncpg, redis, PyYAML (dev: pytest, pytest-asyncio, ruff, mypy, types-pyyaml); OPTIONAL psycopg[binary], testcontainers, greenlet. **Version enforcement (FATH-V5-003):** the majors are not name-implied — A15 verifies the exact declared specifiers (`pydantic>=2,<3`, `pydantic-settings>=2,<3`, `sqlalchemy>=2,<3`) AND the resolved `uv.lock` majors (= 2) for those three packages. Each later dependency (fastapi, langgraph, prefect, unstructured, …) enters ONLY with the task that uses it, with the doc that specifies it cited in that task's plan (dependency discipline).
...
**Pipeline (identical for push and PR events, bound to exact SHA):** the deterministic step order is fixed in TASK-001_PLAN §4.10 (runtime-version checks, **`uv lock --check` then `uv sync --locked`** — `--frozen` does not verify lock freshness per official uv docs (FATH-V4-004) — + three-set dependency contract **with pre-bound version enforcement for pydantic/pydantic-settings/sqlalchemy (FATH-V5-003)**, lint/type, **governed-base diff-boundary check** (`git diff --name-status --no-renames` against the controller-recorded task base SHA, event-independent — FATH-V4-005), migration cycle with three zero-row measurement points, expected-schema oracle **incl. definition-level index comparison via pre-bound `pg_get_indexdef` strings (FATH-V5-002)** and expected-tree oracle, seed-boundary scans incl. the **AST-allowlist migration DML detector with committed negative fixtures (FATH-V5-001)**, scoped AMENDMENT-001 scan, checksum-verified pinned gitleaks). Every third-party GitHub Action is pinned by full commit SHA (mutable-tag defect class — FATH-V5-004 sweep).
...
- Independent reviewer: `plan-reviewer` (GPT-5.6 Sol, 1M, Max reasoning, fresh read-only context). Outcomes APPROVE / REJECT / BLOCKED; max 3 attempts per Escalation Policy §8; **this is review sequence 2, attempt 3 of 3 — FINAL: a rejection becomes BLOCKED_FOR_SALIM** (sequence authorized by Escalation Policy §6 after the human-approved AMENDMENT-002; seq2 attempt-1 findings FATH-V4-001..009 adjudicated in `REMEDIATION-3_ADJUDICATION.md`; seq2 attempt-2 findings FATH-V5-001..005 adjudicated in `REMEDIATION-4_ADJUDICATION.md`; all prior records preserved unchanged in `.delivery/reviews/` and BUILD_STATE history).
