...
**Plan ID:** FATH-BOOTSTRAP-PLAN v4 (fresh planning cycle under AMENDMENT-002 — adjudications in `REMEDIATION-1_ADJUDICATION.md`, `REMEDIATION-2_ADJUDICATION.md`, `V4_ADJUDICATION.md`)
...
**Status:** PLAN_READY_FOR_INDEPENDENT_REVIEW (plan_review_attempt **1 of 3 — fresh review sequence** per Escalation Policy §6: AMENDMENT-002 is a human-approved authority change that resolved FATH-P3-001, the blocker that invalidated the previous attempt sequence; attempts V1–V3 and their reviews remain preserved unchanged) — this plan is NOT self-approved and authorizes no implementation until independent Sol review returns APPROVE.
...
Target-state tree: `PROJECT_MAP.md §2` (doc 16 + control-plane additions `.delivery/`, `.github/workflows/`, plus `golden/`, `scripts/`, `Makefile` required by docs 27/28/31). Scaffolding is delivered incrementally by bounded tasks (ROADMAP.md); TASK-001 creates the package skeleton, config, DB foundation, and CI (TASK-001_PLAN.md v4).
...
- **Python:** uv-managed `pyproject.toml` (`requires-python = ">=3.11,<3.12"` — PROPOSED baseline) + committed `uv.lock`. TASK-001 dependency baseline is minimal and closed (exact permitted/prohibited lists: TASK-001_PLAN §4.9, mechanically checked by A15): pydantic v2, pydantic-settings, SQLAlchemy 2, alembic, asyncpg, psycopg[binary], redis, PyYAML; dev: pytest, pytest-asyncio, ruff, mypy, types-pyyaml, optional testcontainers/greenlet. Each later dependency (fastapi, langgraph, prefect, unstructured, …) enters ONLY with the task that uses it, with the doc that specifies it cited in that task's plan (dependency discipline).
...
4. **Source identity model — SPECIFIED by AMENDMENT-002 (docs/34), replacing the V3 DERIVED/PROPOSED reconciliation and closing FA-OPEN-021 and FATH-P3-001:**
   ...
   - **Complete per-surface propagation (docs/34 §12): `PROPAGATION_MAP.md`** — 69 mapped surfaces across tables, columns, FKs, Pydantic models, event payloads, API/UI/crawler/retrieval/provenance/poisoning/onboarding contracts, YAML/catalogs, and supplementary representations (Redis budget keys, metrics/log fields, CLI args, AGE node properties, belief-calibration dicts), each with citation and classification. Binding on all task plans.
   - **Schema oracle (docs/34 §13): TASK-001_PLAN A4** — independently derived expected-schema fixture proving UUID PK, unique non-null slug, correct UUID FK targets, no `sources` table, no FK targeting slug, exact pinned defaults/types/boundaries.
...
**Pipeline (every push/PR, bound to exact SHA):** the deterministic step order is fixed in TASK-001_PLAN §4.10 (runtime-version checks, `uv sync --frozen` + dependency contract, lint/type, diff-boundary check, migration cycle with three zero-row measurement points, expected-schema and expected-tree oracles, seed-boundary scans, scoped AMENDMENT-001 scan, pinned gitleaks).
...
- **Negative evidence:** for consequential logic, tests must fail when a sign/threshold/field is wrong (oracle policy §11) — e.g. audit chain tamper detection, wrong-hash rejection, budget breach continuing would fail, `sources`-table presence or slug-targeting FK would fail the A4 oracle.
...
- Independent reviewer: `plan-reviewer` (GPT-5.6 Sol, 1M, Max reasoning, fresh read-only context). Outcomes APPROVE / REJECT / BLOCKED; max 3 attempts per Escalation Policy §8; **this is attempt 1 of 3 in the fresh review sequence** authorized by Escalation Policy §6 after the human-approved AMENDMENT-002 (prior V1–V3 sequence exhausted, escalated, resolved by owner decision, and preserved unchanged in `.delivery/reviews/` and BUILD_STATE history).
