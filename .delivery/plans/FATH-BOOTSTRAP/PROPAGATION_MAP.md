...
**Project:** FATH_AUTOPILOT · **Scope:** FATH-BOOTSTRAP (plan V4) · **Role:** CHIEF_ARCHITECT / TASK_PLANNER (Claude Fable 5, 1M, Thinking ON, Max — inherited runtime)
...
**Method (§12 compliance):** the complete corpus (docs 00–33 + README; combined file reference-only) was read in full and additionally swept mechanically for every occurrence of `source_id`, `source_ids`, `sources(`, `slug`, `source_slug`, `source_name`, `source_registry`, the doc-24/30 slug literals, and source-keyed dict/label structures. Every surface is mapped individually below with its citation. **No blanket `str`-type assumption is used anywhere** — each `str`-typed surface is individually classified.
...
| # | Surface | Citation | Mapping | Class | Owning task |
|---|---|---|---|---|---|
| PM-B1 | `source_registry.source_id` | docs/03 line 74; docs/34 §2 | `UUID PRIMARY KEY`, immutable canonical identity | SPECIFIED (R1) | TASK-001 |
| PM-B2 | `source_registry.slug` | docs/34 §3 (column name given by the amendment itself) | `TEXT NOT NULL UNIQUE`; houses the docs-24 §1 / 29 / 30 textual identifiers (PM-M1..M3) | SPECIFIED (R2, R10) | TASK-001 |
...
| PM-B4 | All other `SourceRegistryRecord` fields as columns (name, source_class, reliability_tier, base_url, …, metadata) | docs/03 lines 73–104 | Columns per doc-03 contract; PG type mapping per column recorded with citations in the A4 `expected_schema.json` fixture (mapping choices DERIVED/PROPOSED per column there) | SPECIFIED fields; DERIVED/PROPOSED type mapping | TASK-001 |
...
## Consequences bound into plan V4

1. **TASK-001** creates `source_registry` exactly per PM-A1/B1/B2/B3/B4 and its §13 oracle (TASK-001_PLAN A4) proves: UUID `source_id` PK; `slug TEXT NOT NULL UNIQUE`; no `sources` table; no FK targets `slug`; exact pinned defaults/types.
2. **TASK-006** implements PM-C2/C3, PM-D10/D14, PM-M1–M5 (values still gated on FA-OPEN-020).
