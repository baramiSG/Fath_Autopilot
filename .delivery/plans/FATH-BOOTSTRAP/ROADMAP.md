...
|---|---|---|---|
| **TASK-001** | Repository foundation: doc-16-conformant scaffold, uv env (Python 3.11), config, Postgres 16+AGE+pgvector & Redis 7 compose (running versions verified), Alembic baseline, source-registry **schema only** per docs/34 identity model (UUID `source_id` PK + unique `slug`; no `sources` table; no seed data — FA-OPEN-020) with the docs/34 §13 expected-schema oracle, SHA-bound CI evidence | — (first implementation task; plan approved with this bootstrap) | FA-REQ-W1-001 (schema portion)/002 (schema portion), W1-018/019/020, AM-003, CP-001/002, AM constraints (no Azure artifact) |
...
| TASK-006 | Registry seed data (`config/sources_seed.yaml` keyed on `slug:` per docs/34 §8 + loader, 16 records) + `source_onboarding_checklists` (doc-29 DDL as superseded by docs/34 §7: UUID FK → `source_registry(source_id)`) + Tier-0 activation evidence + Access Guard (doc-03 rules 1–10) + registry service; every decision persisted; non-active denied. **GATED on FA-OPEN-020 (Salim-approved seed value table + Tier-0 onboarding checklists) — dispatch without it is BLOCKED_FOR_SALIM** | 001, 002, 004; FA-OPEN-020 | FA-REQ-W1-001 (seed portion)/002/003/004, AM-003 |
