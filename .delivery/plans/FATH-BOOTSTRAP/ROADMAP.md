...
**Baseline:** `ae5a4ea7db30d9ba243e29c98424702f5e0fb7a1` · **Amendment commits:** `234a24d9e012710352d35aa5b1314a6395614945` (docs/33) · `af00923853e0234da403231258c822b949e9da00` (docs/34)

> **WARNING.** This roadmap decomposes authoritative scope into governed increments. It creates no requirements and implies no completion: nothing on it is done until a candidate is independently reviewed, merged, and evidenced. Authoritative scope lives in `docs/` (AMENDMENT-002 `docs/34` and AMENDMENT-001 `docs/33` at top of precedence — 34 wins over everything earlier where conflicting — then `docs/24`, then the corpus). Weeks 2–6 entries are COARSE and MUST be re-decomposed into bounded task contracts by the planner at each week's task-planning time, re-reading the cited canonical sections. Requirement IDs reference `REQUIREMENTS_TRACEABILITY.json` (derived internal identifiers).
...
- **AMENDMENT-002 verifier checklist applies to every task from TASK-001 onward:** `source_registry` only canonical source table (no `sources` table); `source_id` UUID PK + `slug` TEXT NOT NULL UNIQUE; every persisted source FK → `source_registry(source_id)` (never slug absent human-approved ADR); no contract carries a slug in a `source_id` field; transport `source_id` = serialized UUID with `source_slug` separate; YAML keys on `slug`. Per-surface obligations for each task: `PROPAGATION_MAP.md` (binding planning material).
...
|---|---|---|---|
| **TASK-001** | Repository foundation: doc-16-conformant scaffold, uv env (Python 3.11, lock freshness via `uv lock --check`), config, Postgres 16+AGE+pgvector & Redis 7 compose (running versions verified), Alembic baseline, source-registry **schema only** per docs/34 identity model (UUID `source_id` PK **with immutability trigger + negative test**; unique `slug`; no `sources` table; no seed data — FA-OPEN-020) with the docs/34 §13 expected-schema oracle **pre-bound in TASK-001_PLAN §6.0**, SHA-bound CI evidence (retention per BOOTSTRAP_PLAN §10.6) | — (first implementation task; plan approved with this bootstrap) | FA-REQ-W1-001 (schema portion)/002 (schema portion), W1-018/019/020, AM-003, CP-001/002, AM constraints (no Azure artifact) |
...
| TASK-006 | Registry seed data (`config/sources_seed.yaml` keyed on `slug:` per docs/34 §8 + loader, 16 records) + `source_onboarding_checklists` (doc-29 DDL as superseded by docs/34 §7: UUID FK → `source_registry(source_id)`) + Tier-0 activation evidence + Access Guard (doc-03 rules 1–10) + registry service; every decision persisted; non-active denied. **Binding acceptance obligation (docs/34 §3; FATH-V4-001): mechanical slug post-activation stability enforcement — trigger forbidding `slug` UPDATE on non-candidate rows except through the audited-migration path this task defines, with negative tests (PROPAGATION_MAP PM-B2).** **GATED on FA-OPEN-020 (Salim-approved seed value table + Tier-0 onboarding checklists) — dispatch without it is BLOCKED_FOR_SALIM** | 001, 002, 004; FA-OPEN-020 | FA-REQ-W1-001 (seed portion)/002/003/004, AM-003 |
...
- Entity Resolver; Knowledge Graph Builder (relational canonical + AGE mirror, ARTICLE_PART_OF_LAW, FDI_TARGETS_COUNTRY); EarlyFactCard/EvidenceGraphExplorer; Week-2 smoke (≥3 facts, ≥5 nodes, ≥3 edges). **The Knowledge-Graph-Builder task plan MUST resolve FA-OPEN-022** (graph `source_refs` structure: docs 27/31/32 vs doc-22 `provenance_fact_ids` DDL — PROPAGATION_MAP PM-J7; any direct persisted source reference is UUID → `source_registry(source_id)`; any column beyond doc-22 DDL needs a human-approved ADR).

## Week 3 — Proactive intelligence (COARSE — re-decompose)

Authority: docs/17 (Week 3), 10 (Connection Agent), 08, 13, 27. Requirements: FA-REQ-W3-001. Change Detector; Anomaly Miner; Connection Agent (0.75/0.55 thresholds); Coverage Auditor v0; investigation proposal workflow; Canvas investigation cards; ≥3 unprompted investigations. Canvas-registry ADR if InvestigationCard delta becomes material (FA-OPEN-001). **The Canvas provenance task plan MUST resolve FA-OPEN-023** (doc-15 UI-card "source object IDs" referent set — PROPAGATION_MAP PM-J6; all referents UUID-keyed, serialized per docs/34 §9).
...
FA-OPEN-001 (Canvas registry, W3/W6 human-approved ADR) · FA-OPEN-002 (similarity threshold, W5 ADR) · FA-OPEN-003 (namespaces, W2 ADR) · FA-OPEN-004 (module paths + LLM-client placement, W2 human-approved ADR per doc 23) · FA-OPEN-009 (frontier-LLM API credentials — Salim, before W2 LLM tasks) · FA-OPEN-010 (RTX 5090 workstation connection + sizing re-derivation — Salim/W2+W4 plans) · FA-OPEN-011 (production auth provider — OPEN per docs/33; Salim, production phase) · FA-OPEN-012 (Comtrade key — Tier-1 activation) · FA-OPEN-018 (mandatory provider designation — reserved to Salim) · FA-OPEN-019 (production secrets/object-store hosting — Salim, production phase) · **FA-OPEN-020 (seed value table + Tier-0 onboarding checklists — Salim; MATERIAL: gates TASK-006 and therefore TASK-007/008/014)** · FA-OPEN-021 — **RESOLVED_BY_AMENDMENT_002** (docs/34: UUID `source_id` PK + unique `slug`; propagation per `PROPAGATION_MAP.md`) · **FA-OPEN-022 (graph `source_refs` structure — W2 Knowledge-Graph-Builder plan; PM-J7)** · **FA-OPEN-023 (doc-15 UI-card "source object IDs" referent set — W3 Canvas provenance plan; PM-J6)** · **GATE-SETUP (Salim; MATERIAL: gates every merge, including TASK-001's)**. None blocks TASK-001 implementation or review; FA-OPEN-020 gates the Week-1 activation chain; GATE-SETUP gates all merges. AMENDMENT-002 changes no task dependency: TASK-006 remains gated on FA-OPEN-020 only (its slug-stability enforcement obligation adds no dependency), and the Week-1 dependency graph is unchanged.
